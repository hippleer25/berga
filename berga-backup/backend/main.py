...
from interactions.interactions import like_article, dislike_article, view_article, bulk_view_articles, save_article, unsave_article
from intelligence.recommendations import get_recommendations
...
from intelligence.affinity import analyze_affinity, boost_affinity
...
from feed.following_structure.structure import following_structure, StructureRequest, list_subscriptions
...
DEFAULT_ORIGINS = {"http://localhost:5173", "http://localhost:5746"}
...
def feed_add(feed_data: FeedRequest, user: dict = Depends(get_current_user)):
    return add.subscribe(user, str(feed_data.url))
...
@app.post("/api/teste-parse")
def teste_parse(feed_data: FeedRequest, user: dict = Depends(get_current_user)):
...
def recents(user=Depends(get_current_user), limit: int = 20, max_days: int = 10, folder_id: Optional[str] = None, feed_sha256: Optional[str] = None):
    return get_recents(user["id"], limit, max_days, folder_id, feed_sha256)
...
async def route_opml_import(
    ...
    result = await opml_import.receive(file, user)
