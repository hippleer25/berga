...
def _parse_date(date_str: str) -> datetime | None:
    """Tenta parsear data de artigo em múltiplos formatos."""
    if not date_str:
        return None

    # ISO 8601 (formato mais comum do DDGS)
    for fmt in [
        "%Y-%m-%dT%H:%M:%S",
        "%Y-%m-%dT%H:%M:%S%z",
        "%Y-%m-%dT%H:%M:%S.%f",
...
REQUEST_TIMEOUT = 8          # segundos por fetch de página
MAX_TEXT_CHARS  = 4000       # limite do texto extraído (para não sobrecarregar a IA)

# Domínios que não são fontes primárias de notícias
BLACKLIST = {
...
# Cabeçalhos para simular um browser real e evitar bloqueios
HEADERS = {
    "User-Agent": (
...
def _score_result(r: dict) -> float:
    ...
    # Penaliza parâmetros de query excessivos (tracking links)
...
def extract_text_from_url(url: str) -> str:
    ...
    try:
        ...
        # readability extrai o bloco principal do artigo como HTML
        ...
        # Strip das tags HTML — IA precisa de texto puro, não de markup
...
def search_articles_online(
    ...
    # Mapeia max_days para o timelimit do DDGS
    ...
    if max_days is not None:
        if max_days <= 1:
            ...
            timelimit = "m"
        # > 30 dias: DDGS não suporta, sem filtro de tempo
    ...
    try:
        raw_results = list(DDGS().news(
            ...
            safesearch="off",
            timelimit=timelimit,
            max_results=limit * 3,   # sobre-fetcha para ter margem após filtros
        ))
    except Exception as e:
    ...
    # Filtragem por min_days em pós-processamento (quando DDGS não filtra)
    ...
    # Converte para formato padrão da aplicação
    articles: list[dict] = []
    for idx, r in enumerate(candidates):
        ...
        # Texto completo (opcional — fetch sob demanda)
        full_text = ""
        ...
        # Score decrescente artificial (DDGS não retorna similaridade)
