...
# ---------------------------------------------------------------------------
# list_subscriptions — feeds + pastas vazias
# ---------------------------------------------------------------------------

def list_subscriptions(user_id: int) -> dict:
    """
    Retorna todos os feeds inscritos COM informação de pasta, mais as pastas
    que ainda não têm nenhum feed — para que pastas recém-criadas apareçam
    imediatamente no painel sem precisar adicionar um feed a elas primeiro.

    Entradas de pasta vazia carregam o flag `_empty_folder: True` para que o
    frontend saiba não renderizar uma linha de feed para elas.
    """
    conn = get_db()
    ...
    try:
        ...
        # Pastas que não aparecem em nenhum feed_folders do usuário
        cursor.execute(
            """
