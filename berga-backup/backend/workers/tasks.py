    ...
    with get_db() as conn:
        ...
        try:
            cursor.execute("""
                ...
                WHERE last_parsed_at < NOW() - INTERVAL 6 HOUR
...
async def parse_feeds_for_user(ctx, user_id: int):
    ...
    await parse_user_all_async(user_id)
...
class WorkerSettings:
    ...
    cron_jobs = [
        cron(refresh_stale_feeds, hour={0, 6, 12, 18}, minute=0),
        cron(refresh_weekly_events, hour={0, 6, 12, 18}, minute=30),
