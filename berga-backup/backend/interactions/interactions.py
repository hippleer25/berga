...
def read_article(user_id: int, item_id: str):
    """Usuário leu o artigo (clique). Peso menor que like."""
...
def unsave_article(user_id: int, item_id: str):
    with get_db() as conn:
        ...
        try:
            ...
            """, (item_id,))
            conn.commit()
            return {"status": "success", "message": "Removido dos salvos"}
...
def bulk_view_articles(user_id: int, item_ids: list):
    if not item_ids:
        return {"status": "success", "message": "Nenhum item"}
    ...
    with get_db() as conn:
        ...
        try:
            ...
            conn.commit()
            return {"status": "success", "message": f"{len(item_ids)} views registradas"}
        except Exception as e:
            conn.rollback()
            return {"status": "error", "message": str(e)}
        finally:
