...
def _fetch_week_vectors(days: int = 7) -> tuple[np.ndarray, list[dict]]:
    ...
    payloads = [{**p.payload, "_point_id": str(p.id)} for p in points]

    logger.info(f"[CLUSTER] {len(points)} articles loaded — vectors shape: {vectors.shape}")
...
def _run_clustering(vectors: np.ndarray, min_cluster_size: int) -> np.ndarray:
    ...
    k = min_cluster_size
    nbrs = NearestNeighbors(n_neighbors=k, metric="euclidean", n_jobs=2).fit(normalized)
    distances, _ = nbrs.kneighbors(normalized)
    kth_distances = np.sort(distances[:, -1])

    eps = float(np.percentile(kth_distances, 10))

    logger.info(
        f"[CLUSTER] eps calculated: {eps:.4f} "
...
def _summarize_cluster(articles: list[dict]) -> str:
    ...
    prompt = (
        "You are a Brazilian newspaper editor. "
