...
# ------------------------------------------------------------------
# Enriquecimento com metadados do feed
# ------------------------------------------------------------------

def _enrich_with_feed_metadata(items: list[dict]) -> list[dict]:
    """Busca feed_title e feed_icon no MySQL e enriquece os itens."""
    if not items:
        return items
...
def get_recents(
    ...
    # Filtro válido mas sem feeds correspondentes → retorna vazio
    ...
    scroll_result = client.scroll(
        ...
        limit=limit * 3,  # margem para exclusões
        with_payload=True,
        with_vectors=False,
    )[0]
