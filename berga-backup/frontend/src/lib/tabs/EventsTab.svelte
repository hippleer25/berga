...
onMount(async () => {
	try {
            const res = await fetch('/api/feed/events', { credentials: 'include' });
...
/* ── Centralizer Logic (Idêntico ao HomeTab) ─────────────── */
    ...
    .search-wrap {
        display: flex;
        align-items: center;
        gap: 12px; /* Espaço seguro entre o texto e o ícone */
    ...
    .search-input {
        flex: 1;
        min-width: 0; /* Impede o input de empurrar o ícone para fora da barra */
    ...
    .search-icon {
        flex-shrink: 0; /* Impede que a lupa encolha ou seja escondida */
