<script lang="ts">
    ...
    import {
        ...
        RefreshCw, Link, GripVertical, ArrowLeft
    } from '@lucide/svelte';
...
{#snippet skeletonHeader()}
    <div class="feed-header feed-header--skeleton" aria-hidden="true">
        ...
        <div class="fh-meta">
            <div class="sk-bar" style="width:120px; height:18px; border-radius:6px"></div>
            <div class="sk-bar" style="width:90px; height:11px; border-radius:4px; margin-top:8px; opacity:.6"></div>
        </div>
        <div class="sk-bar fh-btn-sk" style="width:72px; height:34px; border-radius:10px; flex-shrink:0"></div>
    </div>
{/snippet}
...
<div class="page-root">
    <div class="main-content">
        ...
        <header class="top-header">
            <button class="back-btn" onclick={() => history.back()} aria-label="{$t('followerstab.back', { default: 'Back' })}">
                <ArrowLeft size={20} />
            </button>
        </header>
        ...
        {:else}
            <header class="feed-header">
                ...
                <div class="fh-meta">
                    <h1 class="fh-title">{$t('followerstab.title')}</h1>
                ...
                <button class="add-feed-btn" onclick={openAddModal} title="{$t('followerstab.addFeed')}">
                    <Plus size={13} strokeWidth={2.5} />
                    <span>{$t('followerstab.add')}</span>
                </button>
            </header>
        {/if}
        ...
        <div class="toolbar">
            ...
            <div class="toolbar-actions">
                <button
                    ...
                    title="{$t('followerstab.refresh')}"
                    onclick={() => loadSubscriptions(true)}
                    aria-label="{$t('followerstab.refresh')}"
                ...
                <button
                    class="tool-btn"
                    title="{$t('followerstab.newFolder')}"
...
{#if showFollowModal && feedToAdd}
    <FollowFeedModal feed={feedToAdd} onclose={closeFollowModal} />
{/if}
...
/* ── Centralizer ─────────────────────────────────────────── */
    ...
    /* ── Profile Header (matches feed-profile pattern) ──────── */
    .feed-header {
        display: flex;
...
font-family: var(--font-page-title);
    ...
    /* ── Add Feed Button (matches follow-btn pattern) ──────── */
    .add-feed-btn {
        flex-shrink: 0;
        display: inline-flex;
        align-items: center;
