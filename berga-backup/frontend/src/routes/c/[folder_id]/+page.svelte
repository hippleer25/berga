<script lang="ts">
    ...
    import { Clock, Sparkles, ArrowLeft, FolderOpen, Rss, FileText } from '@lucide/svelte';
...
function formatDate(iso: string | null): string {
    ...
    async function loadFolderInfo() {
        ...
        infoError = '';
        try {
            const res = await fetch(`/api/folder-info/${folderId}`, { credentials: 'include' });
            if (res.status === 401) {
            ...
            <header class="folder-header">
                ...
                <div class="fh-meta">
                    ...
                    <div class="fh-sub-row">
                        ...
                        <span class="fh-stat" title="{$t('folder.posts', { default: 'Posts' })}">
                            <FileText size={10} strokeWidth={2} />
                            {folderInfo.entries_count.toLocaleString()}
                        </span>
                    </div>
