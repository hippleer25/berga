<script lang="ts">
	...
	async function importOpml(e: Event) {
		...
		try {
			...
			if (!res.ok) {
				...
				throw new Error(data.message || `${get(t)('settings.serverError').replace('{status}', res.status)}`);
			}
	...
	async function fetchAllArticles() {
		...
		fetchError = '';
		try {
			const res = await fetch('/api/parse-user-all', { method: 'GET', credentials: 'include' });
			if (!res.ok) {
				const data = await res.json().catch(() => ({}));
				throw new Error(data.message || `${get(t)('settings.serverError').replace('{status}', res.status)}`);
			}
