...
.post-card:active:not(.sel-mode) {
  background: color-mix(in oklch, var(--color-base-content) 8%, transparent);
    ...
    .feed-title {
        font-size: 11.5px;
        font-weight: 700;
        color: var(--color-accent); /* Uses theme accent */
        white-space: nowrap;
        overflow: hidden;
        text-overflow: ellipsis;
        /* The trick to prevent early truncation: flex and min-width */
        flex: 1 1 auto;
        min-width: 0;
    }
...
.action-btn:active {
  transform: scale(0.88);
    ...
    .action-active {
        color: var(--color-error) !important; /* Theme red for Like */
    }
    /* Dislike will use the default content color when active, to avoid visual pollution */
