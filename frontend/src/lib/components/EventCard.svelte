...
<style>
    ...
    /* Subtle background when expanded */
...
/* Event Title in Serif (Maximum Highlight) */
.event-title {
...
.article-count {
  ...
  color: var(--color-accent);
    ...
    .expanded-list {
        margin-left: 24px; /* Indentation to create visual hierarchy */
        padding-bottom: 12px;
...
.mini-date { font-size: 11px; color: color-mix(in oklch, var(--color-base-content) 35%, transparent); }
    ...
    /* Title (Sans-serif and lighter to differentiate from Event Title) */
.mini-title {
font-family: var(--font-post-title);
        font-size: 13.5px;
        font-weight: 500;
